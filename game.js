...
function create() {
    ...
    for (let i = 0; i < (drawDistance + 50) / 5 ; i++) {
         ...
         roadsideObjects.push({
             ...
             worldZ: cameraZ + (drawDistance + Math.random() * 50) * segmentLength, // Place ahead of initial camera
...
function update(time, delta) {
    ...
    if (isGameOver) {
        if (Phaser.Input.Keyboard.JustDown(restartKey)) {
            restartGame.call(this); // Use .call(this) if restartGame needs scene context, though not for reload
        }
        return;
    }
